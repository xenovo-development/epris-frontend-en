      ...
      <div class="sidebar-widget-content">
        <ul class="sidebar-widget-list-post">
          <li v-for="(blog, i) in blogData" :key="i">
            <span class="cate">
              <NuxtLink
                :to="`/fachwissen/categories/${cat}`"
                v-for="(cat, i) in blog.categories.slice(0, 1)"
                :key="i"
              >
                {{ cat }}
              </NuxtLink>
    ...
    <div class="sidebar-widget">
      <div class="sidebar-widget-content">
        <div
          ...
          data-overlay="0.7"
          :style="{ backgroundImage: `url('/images/bg/breadcrumb-bg.jpg')` }"
        >
